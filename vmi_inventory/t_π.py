...
t_values = np.linspace(15, 28, 10)  # 调拨价格范围
mean_d = 220
...
# 调拨部分积分：部分1，涉及两个积分项
def integral_part_1(t, q_i, q_neg_i):
    # 计算第一个积分项
    def integrand1(x, y):
...
# 调拨部分积分：部分2，涉及第二个积分项
def integral_part_2(t, q_i, q_neg_i):
    # 计算第二个积分项
    def integrand2(x, y):
...
# 库存共享成本部分：部分1
def inventory_sharing_part_1(t, q_i, q_neg_i):
    def integrand1(x, y):
...
# 库存共享成本部分：部分2
def inventory_sharing_part_2(t, q_i, q_neg_i):
    def integrand2(x, y):
...
# 总利润函数
def profit(t, q_i, q_neg_i):
    part_1 = sales_profit(q_i)
    #D > q
    part_2 = abs(integral_part_1(t, q_i, q_neg_i))
    part_3 = abs(integral_part_2(t, q_i, q_neg_i))
    print(f'part_2 = {part_2}, part_3 = {part_3}')
    #D < q
    part_4 = abs(inventory_sharing_part_1(t, q_i, q_neg_i))
    part_5 = abs(inventory_sharing_part_2(t, q_i, q_neg_i))
...
# 计算不同调拨价格下的利润
profits_D_greater_than_q = [profit(t, q_i, q_neg_i) for t in t_values]

mean_d = 180
profits_D_less_than_q = [profit(t, q_i, q_neg_i) for t in t_values]
...
# D > q的利润曲线
plt.subplot(1, 2, 1)
plt.plot(t_values, profits_D_greater_than_q, label="D=220,q=200(需求量大于订购量)",marker='D', markersize=8,)
...
# D < q的利润曲线
plt.subplot(1, 2, 2)
plt.plot(t_values, profits_D_less_than_q, label="D=180,q=200(需求量小于订购量)", color="orange",marker='^', markersize=8,)
plt.xlabel("调拨价格t",fontsize=14)
plt.ylabel("药店利润",fontsize=14)
